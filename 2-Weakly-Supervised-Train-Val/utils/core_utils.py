...
from models.model_hierarchical_mil import *
...
def train(datasets, cur, args):
    ...
    elif args.model_type == 'mean_linear':
        model = MeanLinear(path_input_dim=args.path_input_dim, n_classes=args.n_classes)
    ...
    for epoch in range(args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:
            if args.attack == 'flag':
                train_flag_loop_clam(epoch, model, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn, args.m, args.step_size)
...
def train_flag_loop_clam(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None,
                         m=3, step_size=2e-3):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.train()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    ...
    for batch_idx, (data, label) in enumerate(loader):
        data, label = data.to(device), label.to(device)
        forward = lambda perturb : model(data + perturb, label=label, instance_eval=True)
        model_forward = (model, forward)
        Y_hat, loss_value, instance_loss_value, total_loss_value, inst_preds, inst_labels = flag_clam(model_forward, data.shape, label, optimizer, device, loss_fn, bag_weight, m, step_size)
        inst_count+=1
        acc_logger.log(Y_hat, label)
        train_inst_loss += instance_loss_value
        inst_logger.log_batch(inst_preds, inst_labels)
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, instance_loss: {:.4f}, weighted_loss: {:.4f}, '.format(batch_idx, loss_value, instance_loss_value, total_loss_value) + 
                'label: {}, bag_size: {}'.format(label.item(), data.size(0)))
...
def train_flag_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):   
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu") 
    model.train()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    train_loss = 0.
